  ...
  public async createOrder(sessionId: string): Promise<string> {
    try {
      ...
      const order = {
        ...
        payment_status: orderData.payment_method === 'CASH_ON_DELIVERY' as PaymentProvider ? 'pending' : 'completed',
        total_amount: orderData.total_amount || 0,
        delivery_cost: orderData.delivery_cost || 0,
        payment_method: orderData.payment_method || 'CASH_ON_DELIVERY' as PaymentProvider,
        items: orderData.items || [],
      ...
      // Marquer le panier comme converti
      // Définition du type pour inclure conversion_date
      type CartUpdateData = Partial<AbandonedCart> & {
        converted_to_order: boolean;
        order_id?: string;
        // Champ supplémentaire non déclaré dans AbandonedCart
